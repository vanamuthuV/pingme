...
export function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-hidden relative">
      {/* Background Grid Pattern */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,var(--border)_1px,transparent_1px),linear-gradient(to_bottom,var(--border)_1px,transparent_1px)] bg-[size:6rem_4rem] opacity-30"></div>
      ...
      <header className="relative z-10 flex items-center justify-between px-6 py-6 md:px-12">
        <div className="flex items-center gap-2">
          ...
          <span className="text-2xl font-bold text-foreground">PingMe</span>
        </div>
        ...
        <Button
          onClick={() => setIsModalOpen(true)}
          className="bg-primary cursor-pointer hover:bg-primary/90 text-primary-foreground px-6 py-2 rounded-full font-medium shadow-sm hover:shadow-md transition-all duration-300"
        >
          Let's Ping ?
        </Button>
      </header>
      ...
      <main className="flex flex-col lg:flex-row items-center px-6 md:px-12 py-8 lg:py-12 min-h-[calc(100vh-100px)]">
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center mb-8 lg:mb-0 lg:pr-8">
          <div className="relative w-full max-w-lg h-96">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative">
                {/* Main sphere */}
                <div className="w-80 h-80 rounded-full border border-border bg-gradient-to-br from-card to-card/80 shadow-2xl relative overflow-hidden">
                  {/* Rotating rings */}
                  <div
                    className="absolute inset-6 border border-border rounded-full animate-spin"
                    style={{ animationDuration: "25s" }}
                  >
                    <div
                      className="absolute inset-6 border border-border rounded-full animate-spin"
                  ...
                  {/* Pulsing inner glow */}
                  <div className="absolute inset-12 bg-gradient-to-br from-card/50 to-card/70 rounded-full animate-pulse opacity-60"></div>

                  {/* Center icon */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-20 h-20 bg-primary rounded-full flex items-center justify-center shadow-xl animate-pulse">
                ...
                {[...Array(12)].map((_, i) => (
                  <div
                    key={i}
                    className="absolute w-4 h-4 bg-muted-foreground rounded-full animate-pulse shadow-lg"
                    style={{
                      top: "50%",
                      left: "50%",
                      transform: `translate(-50%, -50%) rotate(${
                ...
                {[...Array(8)].map((_, i) => (
                  <div
                    key={`inner-${i}`}
                    className="absolute w-2 h-2 bg-muted rounded-full animate-bounce"
                    style={{
                      top: "50%",
                      left: "50%",
                      transform: `translate(-50%, -50%) rotate(${
                ...
                <div className="absolute inset-0">
                  {[...Array(8)].map((_, i) => (
                    <div
                      key={i}
                      className="absolute w-px bg-gradient-to-t from-transparent via-muted-foreground to-transparent opacity-40"
                      style={{
                        height: "280px",
                        top: "50%",
                        left: "50%",
                ...
                {[...Array(6)].map((_, i) => (
                  <div
                    key={`particle-${i}`}
                    className="absolute w-1 h-1 bg-muted-foreground rounded-full animate-ping opacity-75"
                    style={{
                      top: `${20 + Math.random() * 60}%`,
                      left: `${20 + Math.random() * 60}%`,
                      animationDelay: `${i * 0.8}s`,
            ...
            {/* Floating elements */}
            <div className="absolute top-12 right-12">
              <div className="w-12 h-12 bg-card rounded-xl shadow-lg animate-bounce flex items-center justify-center hover:shadow-xl transition-shadow duration-300">
                <Zap className="h-4 w-4 text-muted-foreground" />
              </div>
            </div>
            <div className="absolute bottom-16 left-12">
              <div className="w-14 h-14 bg-primary rounded-xl shadow-lg animate-pulse flex items-center justify-center hover:shadow-xl transition-shadow duration-300">
                <Shield className="h-6 w-6 text-primary-foreground" />
              </div>
            </div>
            <div className="absolute top-24 left-16">
              <div className="w-8 h-8 bg-muted rounded-full shadow-lg animate-ping"></div>
            </div>
            <div className="absolute bottom-24 right-16">
              <div className="w-10 h-10 bg-secondary rounded-xl shadow-lg animate-bounce delay-300 flex items-center justify-center hover:shadow-xl transition-shadow duration-300">
            ...
            {/* Additional floating elements for richness */}
            <div className="absolute top-1/3 right-4">
              <div className="w-6 h-6 bg-muted-foreground rounded-lg shadow-md animate-pulse delay-500 opacity-80"></div>
            </div>
            <div className="absolute bottom-1/3 left-4">
              <div className="w-5 h-5 bg-muted rounded-full shadow-md animate-bounce delay-700 opacity-70"></div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/2 lg:pl-8">
          <div className="space-y-8">
            ...
            {/* Features Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center gap-3 p-4 bg-card rounded-xl border border-border hover:shadow-sm transition-all duration-200">
              ...
              <div className="flex items-center gap-3 p-4 bg-card rounded-xl border border-border hover:shadow-sm transition-all duration-200">
                ...
                <span className="font-medium text-foreground">Group Chats</span>
              </div>
              <div className="flex items-center gap-3 p-4 bg-card rounded-xl border border-border hover:shadow-sm transition-all duration-200">
                <div className="w-10 h-10 bg-primary rounded-lg flex items-center justify-center">
                  <MessageCircle className="h-5 w-5 text-primary-foreground" />
            ...
            {/* Quote */}
            <blockquote className="text-base md:text-lg text-muted-foreground border-l-4 border-border pl-6 italic">
              "The future of communication is here. Simple, secure, and
              lightning fast."
            </blockquote>
            ...
            <div className="pt-2">
              <Button
                onClick={() => setIsModalOpen(true)}
                size="lg"
                className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-4 rounded-full font-medium shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-base md:text-lg"
              >
                Get Started Now <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
            </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-md bg-card">
          ...
          <div className="flex flex-col items-center space-y-6 py-6">
            ...
            >
              <Button className="w-full bg-card hover:bg-card/90 text-foreground border border-border shadow-sm font-medium py-3 px-4 rounded-lg transition-colors duration-200 cursor-pointer">
                <svg className="w-5 h-5 mr-3" viewBox="0 0 24 24">
                  <path
                    fill="#4285F4"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
